...
from ._quality_rules import QUALITY_RULES_AR, SYNTHESIZER_RESOLUTION_RULES_AR

logger = logging.getLogger(__name__)

SYSTEM_PROMPT = QUALITY_RULES_AR + "\n\n" + SYNTHESIZER_RESOLUTION_RULES_AR + "\n\n---\n\n" + """أنت صانع القرار النهائي في لجنة دراسة جدوى الوساطة التجارية.
...
** مهم جداً: يجب أن ترد بصيغة JSON فقط بالهيكل التالي **
{
  ...
  "numerical_inconsistencies": [
    {
      "field": "TAM",
      "values_reported": "وكيل السوق: 5.25M د.ب — وكيل النماذج: 25-30M د.ب",
      "resolution": "أعتمد رقم وكيل السوق لأنه مبني على عدد شركات + متوسط عقد، رقم النماذج مبني على نسبة من GDP قطاعي شامل غير دقيق",
      "adopted_value": "5-7 مليون د.ب"
    }
  ],
  "misleading_citations": [
    {
      "agent": "اسم الوكيل",
      "citation": "نص الاستشهاد المضلل",
      "issue": "لماذا هو مضلل (مثلاً: استخدم نمو قطاع شامل كدليل لفرع منه)"
    }
  ],
...
بناءً على التحليلات الستة أعلاه، قدم حكمك النهائي حول جدوى الوساطة التجارية في هذا القطاع، مع تحديد النموذج الأمثل."""
        ...
        try:
            ...
            return json.dumps({"summary": content, "consensus": [], "conflicts": [], "verdict": "تحتاج دراسة أعمق", "overall_score": 0, "score_justification": "", "recommended_model": "", "model_justification": "", "advice": [], "weighted_breakdown": {}, "contradictions_found": [], "critical_data_gaps": [], "numerical_inconsistencies": [], "misleading_citations": [], "overall_confidence": 0, "advisor_opinion": ""}, ensure_ascii=False)
